    ...
    const send = async () => {
        try {
            ...
            const interval = setInterval(() => {
                ...
                ].content += fullText.slice(i, i + speed);
    ...
    return (
        <>
            <section className=" min-h-screen relative h-full pt-12 overflow-hidden w-full flex items-center ">
                                        ...
                                        <div className="  mt-5 w-fit h-fit flex-col ml-10  overflow-hidden   gap-3 flex justify-start items-start">
                                            ...
                                            {chatList.map((chat, index) => (
                                                <div onClick={() => setActiveChatIndex(index)} key={index} className={` bg-[#242424] ${activeChatIndex === index ? "dark:bg-[#242424] bg-[#242424]/50 " : "bg-[#242424]/20"} justify-between flex-row h-10 w-56 flex items-center px-6 rounded-4xl`}>
                                                    <h2>{chat.title}</h2>
                                                    <i onClick={() => { deletechat(index) }} className=" text-red-500 font-semibold cursor-pointer hover:dark:text-red-300 hover:text-red-300 transition-all active:text-red-300 active:dark:text-red-300 fa-solid fa-trash-can"></i>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                ...
                <section className=" flex-row  flex dark:bg-black  xl:h-[93vh] h-[92vh] w-full bg-white ">
                    <Toaster />
                    {
                        loadingchat ?
                    ...
                    {/* side Area */}
                    {!side ?
                        <div className=" h-full w-80 bg-[#F9FAFB] dark:bg-black overflow-y-scroll">
